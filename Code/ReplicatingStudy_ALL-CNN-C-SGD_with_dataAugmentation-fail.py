...
def trainModel(model, iteration_learning_rate, number_of_epochs, folder_name):
    ...
    datagen.fit(x_train)
    # datagen.fit(x_test)
    x_train = datagen.standardize(np.copy(x_train))
    ...
    #             epochs=number_of_epochs, validation_data = (x_test,y_test), callbacks = [tf.keras.callbacks.LearningRateScheduler(lambda epoch: lr_schedule(epoch, iteration_learning_rate)), cp_callback])
...
initializeTraining(0.01, "replicating_study_with_whitening", 350)
